...
tab csdmard if rheum_appt_date!=. & csdmard_date<rheum_appt_date & psa_code==1 
...
tab csdmard_hcd if rheum_appt_date!=. & csdmard_hcd_date<rheum_appt_date & psa_code==1  
...
tab mtx if rheum_appt_date!=. & mtx_date<rheum_appt_date & psa_code==1 //how many patients had precriptions pre-rheum appt
...
tab mtx_hcd if rheum_appt_date!=. & mtx_hcd_date<rheum_appt_date & psa_code==1 //how many patients had precriptions pre-rheum appt
...
tab biologic_6m //for all EIA patients
bys eia_diagnosis: tab biologic_6m 
gen biologic_12m=1 if time_to_biologic<=365 & time_to_biologic!=.
recode biologic_12m .=0
tab biologic_12m //for all EIA patients
bys eia_diagnosis: tab biologic_12m 

**Biologic time categories (by year)
bys diagnosis_year: tab biologic_6m //for all EIA patients
bys diagnosis_year eia_diagnosis: tab biologic_6m //for each diagnosis
bys diagnosis_year: tab biologic_12m //for all EIA patients
bys diagnosis_year eia_diagnosis: tab biologic_12m //for each diagnosis
