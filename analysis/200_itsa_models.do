...
**Sensitivity analyses using time from last GP (pre-rheum appt) to rheum appt (all diagnoses)
preserve
...
**Time from rheum appt to shared care csDMARD for RA patients (not including MTX high-cost drug) - may need to cut sooner to give sufficient time for DMARD
preserve
asdoc tabstat time_to_csdmard if ra_code==1, by(mo_year_diagn) stat(count p50 p75 p25 mean sd) save($projectdir/output/tables/csDMARD_delay_RA.doc) dec(0)
...
**Time from rheum appt to shared care csDMARD for PsA patients (not including MTX high-cost drug) - may need to cut sooner to give sufficient time for DMARD
preserve
asdoc tabstat time_to_csdmard if psa_code==1, by(mo_year_diagn) stat(count p50 p75 p25 mean sd) save($projectdir/output/tables/csDMARD_delay_PsA.doc) dec(0)
...
**Time from rheum appt to shared care MTX for RA patients (not including MTX high-cost drug) - may need to cut sooner to give sufficient time for DMARD
preserve
asdoc tabstat time_to_mtx if ra_code==1, by(mo_year_diagn) stat(count p50 p75 p25 mean sd) save($projectdir/output/tables/MTX_delay_RA.doc) dec(0)
...
**Time from rheum appt to shared care MTX for PsA patients (not including MTX high-cost drug) - may need to cut sooner to give sufficient time for DMARD
preserve
asdoc tabstat time_to_mtx if psa_code==1, by(mo_year_diagn) stat(count p50 p75 p25 mean sd) save($projectdir/output/tables/MTX_delay_PsA.doc) dec(0)
...
preserve
asdoc tabstat time_to_biologic if has_12m_follow_up==1, by(mo_year_diagn) stat(count p50 p75 p25 mean sd) save($projectdir/output/tables/Biol_delay_all.doc) dec(0)
keep if time_to_biologic!=. & has_12m_follow_up==1 //drop those with no rheum appt and/or biologic prescription
...
preserve
asdoc tabstat time_to_biologic if ra_code==1 & has_12m_follow_up==1, by(mo_year_diagn) stat(count p50 p75 p25 mean sd) save($projectdir/output/tables/Biol_delay_ra.doc) dec(0)
keep if time_to_biologic!=. & has_12m_follow_up==1 & ra_code==1 //drop those with no rheum appt and/or biologic prescription
collapse (p50) p50_biologic_delay=time_to_biologic (p75) p75_biologic_delay=time_to_biologic (p25) p25_biologic_delay=time_to_biologic (mean) mean_biologic_delay=time_to_biologic (sd) sd_biologic_delay=time_to_biologic (count) n_biologic_delay=time_to_biologic, by(mo_year_diagn)
...
preserve
asdoc tabstat time_to_biologic if psa_code==1 & has_12m_follow_up==1, by(mo_year_diagn) stat(count p50 p75 p25 mean sd) save($projectdir/output/tables/Biol_delay_PsA.doc) dec(0)
keep if time_to_biologic!=. & has_12m_follow_up==1 & psa_code==1 //drop those with no rheum appt and/or biologic prescription - may need to cut sooner to give sufficient time for DMARD
collapse (p50) p50_biologic_delay=time_to_biologic (p75) p75_biologic_delay=time_to_biologic (p25) p25_biologic_delay=time_to_biologic (mean) mean_biologic_delay=time_to_biologic (sd) sd_biologic_delay=time_to_biologic (count) n_biologic_delay=time_to_biologic, by(mo_year_diagn)
...
preserve
asdoc tabstat time_to_biologic if anksp_code==1 & has_12m_follow_up==1, by(mo_year_diagn) stat(count p50 p75 p25 mean sd) save($projectdir/output/tables/Biol_delay_anksp.doc) dec(0)
keep if time_to_biologic!=. & has_12m_follow_up==1 & anksp_code==1 //drop those with no rheum appt and/or biologic prescription - may need to cut sooner to give sufficient time for DMARD
collapse (p50) p50_biologic_delay=time_to_biologic (p75) p75_biologic_delay=time_to_biologic (p25) p25_biologic_delay=time_to_biologic (mean) mean_biologic_delay=time_to_biologic (sd) sd_biologic_delay=time_to_biologic (count) n_biologic_delay=time_to_biologic, by(mo_year_diagn)
